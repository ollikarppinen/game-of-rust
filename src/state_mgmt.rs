...
fn update_camera(state: &mut State, config: &Config) -> () {
    ...
    let old_cx = state.camera_position_x / state.cell_width;
    let old_cy = state.camera_position_y / state.cell_height;
    ...
    state.camera_position_x += (old_cx - state.camera_position_x / state.cell_width) * state.cell_width;
    state.camera_position_y += (old_cy - state.camera_position_y / state.cell_height) * state.cell_height;
}
